package com.nearor.demo.animation.launcher;
...
import com.nearor.demo.animation.R;
...
//        }, 2400);
    ...
    private void setAnimation(final ImageView target, ViewPath path1, ViewPath bezierPath2) {
        //左右平移
        ...
        //贝塞尔曲线
        ObjectAnimator bezierAnim2 = ObjectAnimator.ofObject(new ViewObj(target), "fabLoc", new ViewPathEvaluator(), bezierPath2.getPoints().toArray());
        bezierAnim2.setInterpolator(new AccelerateDecelerateInterpolator());


        //组合动画
        addAnimation(redAnim1, bezierAnim2, target);

    }

    private void addAnimation(ObjectAnimator animator1, ObjectAnimator bezierAnim, ImageView target) {
        ...
        all2.playTogether(alpha, scaleX, scaleY, bezierAnim);
        //all2.addListener(new AnimEndListener());

        AnimatorSet all = new AnimatorSet();
        all.playSequentially(animator1, all2);
        all.addListener(new AnimEndListener(target));
        all.start();
